...
% Define os fatos que sao dinâmicos, ou seja, podem ser removidos e adicionados durante a execução do programa
:- dynamic([
    posicao_agente/1,
    posicao_coracoes/1,
    posicao_garrafas/1,
...
% Passar todas as posições dentro de listas (Ex: [[0,0], [1,1]])

% iniciar([[4,0]],[[0,2],[1,0],[2,4],[3,9]],[[1,3],[2,5],[3,2],[4,1]],[],[],[]).
...
obter_posicao_adjacente_livre([Linha,Coluna],FatorMultiplicador,[Linha,C]) :-
    ...
    (not(tem_obstaculo([Linha,C2])), C is C2)), % se a primeira regra do OU falha é porque a posição é um obstáculo, logo 
    % verificamos se a posição adjacente ao obstáculo é outro obstáculo, caso seja não é possível pular
    dentro_tabuleiro([Linha,C]),!. % O corte faz o prolog executar a regra obter_posicao_adjacente_livre somente uma vez
...
% Espaço - 32 | Estado visitado - 42 | Agente - 65 | Brutus - 66 |
% Coração - 67 | Elevador - 69 | espinaFre - 70 | Garrafa - 71
imprimir_informacao(LinhaAtual,ColunaAtual) :-
